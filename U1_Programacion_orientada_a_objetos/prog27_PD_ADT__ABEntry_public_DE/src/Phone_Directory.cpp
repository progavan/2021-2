...
:size(0),capacity(5)
{
    ...
    the_directory = new ABEntry[capacity];
}
  ...
  if(in){// Si existe el flujo
    ...
    while(getline(in,name)){
        if(getline(in,number)){
          if(getline(in,domicilio)){
            //add(name,number);
            add(name,number,domicilio);
          }
        }
    }
    // Cerrar el archivo
...
std::string Phone_Directory::lookup_entry(
  ...
  if(index != -1){/** Si se encontr\'o name */
   the_directory[index].update_num_TimesConsulted();
...
int Phone_Directory::lookup_num_TimesConsulted(
            const std::string& Name) const{
  int index = find(Name);
  if(index != -1){/** Si se encontr\'o name */
    return
    the_directory[index].get_num_TimesConsulted();
  }else{/** index es -1 */
    return -1;
  }
}
...
void Phone_Directory::mostrar(){
  ...
  #if 1 //LMC 2021.11.18
   std::cout<<the_directory[i].get_domicilio()<<std::endl;
  #endif // 1
  }
}
#endif // 1
...
//           new Directory_Entry[capacity];
  ABEntry *new_directory =
           new ABEntry[capacity];
...
/**
  ...
  y vamos a remover la entrada con \'indice 1,
